...
session_start(); 
	...
	$email=$_SESSION['namEmail'];

	$sqln='select status from cart_offline where email="'.$email.'"'; 
	...
	// generate a uniqid here and store it in the database as well here 
	$idq=uniqid(); 
	$sql1='insert into cart_offline(cid,bill,cdate,email) values("'.$idq.'",'.$price.',"'.date('Y-m-d H:i:s',$time).'","'.$email.'")';
	//echo $sqln; 
	//echo $count;
	//echo $sql1; 
	if($count==1){
	...
	/*
	$sql3='select cid from cart_offline where bill'; 

	$res=$mysqli->query($sql3);
	
	//$i=$res->num_rows; 
	
	while($rowf=$res->fetch_array()){
		$i=$rowf[0]; 	
	}
	//$i+=1; 
	 */
	...
	$sql2='insert into productoffline(cid,id,quantity) values("'.$idq.'",'.$id.','.$_SESSION['cart'][$id]['quantity'].')';
	//echo $sql2; 
		$mysqli->query($sql2) ; 
	//	echo $sql2; 
...
$sql="select * from products2 where id in (";
